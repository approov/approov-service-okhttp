...
### Fixed
- **Redirects are reclassified.** A request redirected by OkHttp is a new network attempt to a new URL. The network layer now strips the protection applied to the original destination (token, trace and status headers, signatures, and the secure string substitutions, whose placeholders are restored) and applies protection afresh for the redirect target: a redirect to a domain Approov does not protect leaves with no Approov headers at all, and a redirect within a protected domain is re-signed over its new target URI. Previously the original destination's headers, including substituted secrets, travelled with the redirect.
